...
class embeddedReachGridNet(GridNet):
    """
    Embedding the Reach pruning into the Q-learning algorithm.
    """
    def __init__(self, adj_matrix, var_matrix, 
                 initial_node=0, destination_node=24, 
                 max_rem_rew=30, discrete_rate=4, reward_value=0, 
                 ...
                 episode_lissage=50000, alpha=0.001, gamma=0.99):
        """
        Reach class works with the associated graph instance.
        """
        # building graph and initializing classes
        graph = StochasticGraph(adj_matrix, var_matrix)
        self.dijkstra = Dijkstra(adj_matrix=adj_matrix)
        self.reach = detReach(graph, self.dijkstra)

        # computing reach values
        self.reach.reach_computation()
    ...
    def learn(self):
        """
        R2L learning function with embedded-Reach pruning at each cycle.
        """
        # initializing data structures
        max_step = [15, 14, 13, 12, 11, 14, 13, 12, 11, 10, 13, 12, 11, 10, 9, 12, 11, 10, 9, 8, 11, 10, 9, 8, 7]
        ...
        # looping on all episodes while the deviation standard is greater than the precision
        while (ep <= self.episode_number) and (dev1 >= self.precision):
            # resetting the graph and re-initializing the environment
            self.reach.reset_graph()
            self.reset()

            # pruning the graph according to reach values computed at the beginning
            self.reach.reach_pruning(self.current_node, self.destination_node)
